...
class AreaCount {
	...
	function getNames ( $array, $nations="" ) {
		if ( $nations != "" ) {
			$nations = preg_replace ( "/,[[:space:]]*$/", "", $nations );
			if ( strstr ( $nations, "," ) ) {
...
class Event {
	...
	function getEvent ( ) {
		...
		for ( $i=0; $i<$this->counter; $i++ ) {
			if ( !preg_match ("/^[[:space:]]*$/", $this->handle[$i]) AND !preg_match ("/^#/", $this->handle[$i]) ) {
...
$health = $_GET['health'];
$vitality = $_GET['vitality'];
$agressivity = $_GET['agressivity'];
$military = $_GET['military'];
$warProb = $_GET['warProb'];
$prefered = $_GET['prefered'];
$negative = $_GET['negative'];
$fixed = $_GET['fixed'];
$path = $_GET['path'];
$num_syls = $_GET['num_syls'];
$num_names = $_GET['num_names'];
